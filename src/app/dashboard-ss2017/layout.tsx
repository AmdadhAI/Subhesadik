...
const navItems = [
  { href: "/dashboard-ss2017", icon: LayoutDashboard, label: "Dashboard" },
  { href: "/dashboard-ss2017/orders", icon: ShoppingCart, label: "Orders" },
  { href: "/dashboard-ss2017/products", icon: Package, label: "Products" },
  { href: "/dashboard-ss2017/categories", icon: Tag, label: "Categories" },
  { href: "/dashboard-ss2017/customers", icon: Users, label: "Customers" },
  { href: "/dashboard-ss2017/content", icon: Settings, label: "Content" },
];

function AdminNavLink({ href, children, isMobile = false }: { href: string; children: React.ReactNode, isMobile?: boolean }) {
  const pathname = usePathname();
  const isActive = pathname === href || (href !== '/dashboard-ss2017' && pathname.startsWith(href));
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (pathname === '/dashboard-ss2017/login') return;
    if (!isLoading && !user) {
      router.replace('/dashboard-ss2017/login');
    }
  }, [isLoading, user, router, pathname]);

  useEffect(() => {
    if (!isLoading && user && !isAdmin && pathname !== '/dashboard-ss2017/login') {
      // If user is loaded, logged in, but not an admin, they are forbidden from admin pages.
    } else if (!isLoading && user && isAdmin && pathname === '/dashboard-ss2017/login') {
      // If a logged-in admin lands on the login page, redirect them to the dashboard.
      router.replace('/dashboard-ss2017');
    }
  }, [isLoading, user, isAdmin, pathname, router]);

  const handleLogout = async () => {
    await signOut(auth);
    router.push('/dashboard-ss2017/login');
  };

  if (pathname === '/dashboard-ss2017/login') {
      ...
      <div className="hidden border-r bg-card md:block">
        <div className="flex h-full max-h-screen flex-col gap-2">
          <div className="flex h-14 items-center border-b px-4 lg:h-[60px] lg:px-6">
            <Link href="/dashboard-ss2017" className="flex items-center gap-2 font-semibold">
      ...
      <div className="flex flex-col">
        <header className="flex h-14 items-center gap-4 border-b bg-card px-4 lg:h-[60px] lg:px-6 sticky top-0 z-40">
          <div className="w-full flex-1">
            <Link href="/dashboard-ss2017" className="flex items-center gap-2 font-semibold md:hidden">
              <span className="font-headline text-primary">Subhe Sadik Admin</span>
            </Link>
          </div>
